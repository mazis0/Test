...
public class BasePage {
    ...
    public MobileElement andScrollToElementUsingUiScrollable(String childLocAttr, String childLocValue) {
        return (MobileElement) ((FindsByAndroidUIAutomator) driver).findElementByAndroidUIAutomator(
                "new UiScrollable(new UiSelector()" + ".scrollable(false)).scrollIntoView("
    ...
    public MobileElement scrollToElement(MobileElement element, String direction) throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            if (find(element, 1)) {
                ...
                swipe(startX, startY, endX, endY, 100000);
            }
        }
        if(!isFound){
...
//    public void scrollUsingTouchActionsByElementsss(MobileElement element, MobileElement endElement) {
//        TouchAction actions = new TouchAction(driver);
//        actions.press(element).waitAction(Duration.ofSeconds(2)).moveTo(endElement).release().perform();
//
//    }

    public static void scrollToElement(AndroidDriver<MobileElement> driver, String elementName) {
